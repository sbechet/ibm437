...
fn glyph_copy(output: &mut [u8], input: &[u8], input_index: usize, input_width_len: usize, input_height_len: usize) {
...
fn extract_9x14(characters: &Vec<char>, ibm437_src: &[u8; 8192]) -> std::io::Result<()>  {
    ...
    let mut font_output: [u8; INPUT_WIDTH_LEN*CHARS_PER_ROW * INPUT_HEIGHT_LEN*(256/CHARS_PER_ROW)] = [0; INPUT_WIDTH_LEN*CHARS_PER_ROW * INPUT_HEIGHT_LEN*(256/CHARS_PER_ROW)];

    for (i, chr) in characters.iter().enumerate() {
        let top = &ibm437_src[8 * i..8 * (i + 1)];
        let bottom = &ibm437_src[0x0800 + 8 * i..0x0800 + 8 * (i + 1) - 2];
        let mut glyph: [u8; INPUT_WIDTH_LEN*INPUT_HEIGHT_LEN] = [0; INPUT_WIDTH_LEN*INPUT_HEIGHT_LEN];
        ...
        glyph_copy(&mut font_output, &glyph, char_offset_impl(*chr), INPUT_WIDTH_LEN, INPUT_HEIGHT_LEN);
    }
    save_9_14("target/ibm437_font_9_14_regular.raw", &font_output)?;
    Ok(())
}

fn save_9_14(filename: &str, font_input: &[u8; 2*CHARS_PER_ROW * 14*(256/CHARS_PER_ROW)]) -> std::io::Result<()> {
    ...
    for y in 0..(256/CHARS_PER_ROW) {
        for x in 0..CHARS_PER_ROW {
            ...
            for gy in 0..14 {
                for gx in 0..9 {
                    let src: u8 = match font_inputb[seek_src_bits_index + gx + gy*CHARS_PER_ROW*16] {
                        true => 1,
                        _ => 0
                    };
    ...
    save_png("doc/ibm437_font_9_14_regular.png", font_outputb.as_raw_slice(), 9*CHARS_PER_ROW, 14*(256/CHARS_PER_ROW))?;
...
fn extract_8x8_regular(characters: &Vec<char>, ibm437_src: &[u8; 8192]) -> std::io::Result<()>  {
    ...
    save_png("doc/ibm437_font_8_8_regular.png", &font_output, 8*CHARS_PER_ROW, 8*(256/CHARS_PER_ROW))?;
...
fn extract_8x8_bold(characters: &Vec<char>, ibm437_src: &[u8; 8192]) -> std::io::Result<()>  {
    ...
    save_png("doc/ibm437_font_8_8_bold.png", &font_output, 8*CHARS_PER_ROW, 8*(256/CHARS_PER_ROW))?;
    Ok(())
}
